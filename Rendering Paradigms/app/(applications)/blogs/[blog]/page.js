...
export const revalidate = 0;//this will disable the isr and the page will be statically generated at build time only
...
export async function generateStaticParams() {//these will be generated at build time
  ...
  const data = await response.json()//now the data for fake api is statically generated at build time and 
...
//one more thing to note if the page has some dynamic data to display then the variable revalidate will work otherwise it will not work
  return data.map((item) => ({ blog: `${item.id}` }))
  // return [
  //   {    blog: "1",
  //   },
  //   {
